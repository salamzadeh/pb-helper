...
class PBHelperServiceProvider extends ServiceProvider
{
    /**
    ...
    {
        $this->publishes([
            __DIR__.'/../config/config.php' => config_path('pb-helper.php'),
        ], 'config');

        Validator::resolver(function ($translator, $data, $rules, $messages = [], $customAttributes = []) {
            return new PBHelperValidator($translator, $data, $rules, $messages, $customAttributes);
        });

        if (config('pb-helper.iran_province')) {
    ...
    {
        $this->mergeConfigFrom(__DIR__.'/../config/config.php', 'pb-helper');
        app(\Faker\Generator::class)->addProvider(new PBHelperFakerProvider(app(\Faker\Generator::class)));
    }
